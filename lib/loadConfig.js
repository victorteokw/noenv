...
module.exports = function(requirer) {
  ...
  possibleLocalFileNames.forEach((l) => {
    possibleConfFileExtensions.forEach((ext) => {
  ...
  possibleDevFileNames.forEach((d) => {
    possibleConfFileExtensions.forEach((ext) => {
      const maybe = path.join(confDir, `${d}.${ext}`);
      fs.existsSync(maybe) && (devFile = maybe) && (devFileName = d);
    });
  });
  if (devFile) {
  ...
  const allFilesInConfDir = fs.readdirSync(confDir);
  let anyfile = allFilesInConfDir.find((f) =>
    possibleConfFileExtensions.includes(path.extname(f).replace('.', '')));
  if (anyfile) {
